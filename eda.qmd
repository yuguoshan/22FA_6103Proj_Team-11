...
```{python}
yesNo = bankdata.groupby('y').size()
yesNo.plot(kind='pie', title='Percentage of yes and no target(term deposit)in dataset', figsize=[8,8],
...
Checking for the entries for non ecistent term
...
There are *36959   unknown* values, so we can directly drop this column.  
